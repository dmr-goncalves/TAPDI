...
namespace SS_OpenCV {
	...
	public struct ColorConversion {
		...
		public ColorConversion(float h, float s, float b) {
			this.a = 0xff;
		...
		public ColorConversion(int a, float h, float s, float b) {
			this.a = a;
			this.h = Math.Min(Math.Max(h, 0), 255);
			this.s = Math.Min(Math.Max(s, 0), 255);
			this.b = Math.Min(Math.Max(b, 0), 255);
		...
		public ColorConversion(Color color) {
			ColorConversion temp = FromARGB(color);
			this.a = temp.a;
			this.h = temp.h;
			this.s = temp.s;
			this.b = temp.b;
		...
		public static Color ShiftHue(Color c, float hueDelta) {
			ColorConversion hsb = ColorConversion.FromARGB(c);
			hsb.h += hueDelta;
		...
		public static Color FromHSB(ColorConversion hsbColor) {
			float r = hsbColor.b;
			float g = hsbColor.b;
			float b = hsbColor.b;
			if(hsbColor.s != 0) {
		...
		public static ColorConversion FromARGB(Color color) {
			ColorConversion ret = new ColorConversion(0f, 0f, 0f);
			ret.a = color.A;
			float r = color.R;
			float g = color.G;
			float b = color.B;
